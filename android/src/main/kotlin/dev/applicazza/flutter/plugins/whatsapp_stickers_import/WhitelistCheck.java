...
public class WhitelistCheck {
    ...
    private static boolean isWhitelistedFromProvider(@NonNull Context context, @NonNull String identifier, String whatsappPackageName) {
        ...
        if (isPackageInstalled(whatsappPackageName, packageManager)) {
            ...
            final Uri queryUri = new Uri.Builder().scheme(StickerContentProvider.CONTENT_SCHEME).authority(whatsappProviderAuthority).appendPath(QUERY_PATH).appendQueryParameter(AUTHORITY_QUERY_PARAM, authority).appendQueryParameter(IDENTIFIER_QUERY_PARAM, identifier).build();
